...
reward_history, actor_losses, critic_losses, hit_rates, eval_scores = train(actor, actor_optimizer, critic,
                                                                            critic_optimizer, num_episodes=NUM_EPISODES, gamma=GAMMA, print_interval=500, eval_interval=2500)

#----Save Results----
with open('reward_history.dill', 'wb') as f:
    dill.dump(reward_history, f)
    
with open('actor_losses.dill', 'wb') as f:
    dill.dump(actor_losses, f)
    
with open('critic_losses.dill', 'wb') as f:
    dill.dump(critic_losses, f)
    
with open('hit_rates.dill', 'wb') as f:
    dill.dump(hit_rates, f)
    
with open('eval_scores.dill', 'wb') as f:
